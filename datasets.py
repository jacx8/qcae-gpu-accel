...
def train_dataset(n_samples = 20, batch_size = 1):
    if NUM_CHANNELS ==1:
        ...
        X_train = CIFAR10(root='./data', train=False, download=True, transform=transforms.Compose([transforms.Resize((IMAGE_SIZE,IMAGE_SIZE)),transforms.ToTensor()]))
...
def test_dataset(n_samples = 20, batch_size = 1):
    if NUM_CHANNELS==1:
        ...
        X_test = CIFAR10(root='./data', train=False, download=True, transform=transforms.Compose([transforms.Resize((IMAGE_SIZE,IMAGE_SIZE)),transforms.ToTensor()]))
